...
def content_sha256(data):
    """SHA-256 of the mathematical content of an archive: run parameters, active primes,
    overlaps and the four ball matrices, in canonical JSON. Unlike the hash of the .gz
    file it does not depend on timestamps, runtime metadata or the stored analytic-bound
    enclosures (which every replay recomputes and checks), so two independent builds can
    be compared by this value alone."""
    keys = ('N', 'M', 'precision_bits', 'horizon', 'log_horizon', 'active_prime_powers',
            'reflected_delay_overlaps', 'matrices')
    canon = json.dumps({k: data[k] for k in keys}, sort_keys=True, separators=(',', ':'))
    return hashlib.sha256(canon.encode()).hexdigest()


def locate(path):
    """Return `path` if it exists; otherwise, if WEIL_ARCHIVES is set, the same path
    relative to the numerics directory under that root (where the unversioned ball
    archives are kept). Raises FileNotFoundError with instructions otherwise."""
    import os
    path = Path(path)
    if path.exists():
        return path
    root = os.environ.get('WEIL_ARCHIVES')
    if root:
        here = Path(__file__).resolve().parent
        try:
            rel = path.resolve().relative_to(here)
        except ValueError:
            rel = path
        cand = Path(root)/rel
        if cand.exists():
            return cand
    raise FileNotFoundError(f'{path} not found. Ball archives are not versioned: rebuild it with '
                            f'certify_arb.py (about one minute at N=128), copy it back from the '
                            f'numerics-archives folder, or set WEIL_ARCHIVES to that folder.')
...
def main():
    ...
    result['matrices_content_sha256']=content_sha256(data)
