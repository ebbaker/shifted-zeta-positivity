...
\large A full-output parity certificate through total horizon $9/5$}
\author{Edward Baker}
\date{Review draft, version 0.1\quad---\quad 9 September 2026}
...
$Q_{0,L}\succeq10^{-26}I$ for every total horizon $0<L\le9/5$.
The outer horizon lies beyond $\log6$: the generator contains the prime-power
...
reflection-positivity and cumulative-storage bounds. The argument does not use
zeta zeros or an assumed positive spectral measure. It establishes a
...
not been completed. The numerical result is isolated in
...
certificate through total length $1.8$, with explicit positive-shift
...
$\sqrt2$ for $2d<L\le3d$. The implementation uses these two exact cases
and the safe bound 2 otherwise.
...
\section{Finite construction and the certificate at \texorpdfstring{$9/5$}{9/5}}\label{sec:computation}
...
The next concrete depth target is total horizon $L=\log7$, using the same
full-output parity construction and adapting the retained space and profile
degree as required. At that endpoint the $n=7$ delay is zero almost
everywhere; a strict increase beyond it activates the next prime. A more
general result would retain structure in the tail resolvent or in the
cumulative storage update instead of reducing all arithmetic tail terms
to separate scalar losses.
