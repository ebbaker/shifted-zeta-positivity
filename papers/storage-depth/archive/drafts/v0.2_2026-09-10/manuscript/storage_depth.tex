...
\usepackage{fancyhdr}
\pagestyle{fancy}\fancyhf{}\fancyhead[L]{\small Cumulative storage and depth extension}\fancyhead[R]{\small Research draft 0.2}\fancyfoot[C]{\thepage}
...
\date{10 September 2026 \quad\textbullet\quad Version 0.2}
...
\maketitle
\begin{abstract}
We study finite-horizon depth extension for the shifted-zeta transfer in the working normalization of the Weil-depth v0.3 manuscript. The cumulative contraction defect admits an exact Cayley congruence whose normalized spatial coupling equals the original storage coupling. A continuation map converts the depth Schur complement into candidate energy minus a quadratic stationarity residual. At the first quarter-step from $\log 7$ toward $\log 8$, an exact rational 128-to-32 polynomial continuation satisfies a computer-assisted residual bound on the entire old form domain: $R^*F^{-1}R\preceq0.9H_J$. A spatial verification head with 256 old and 32 new modes, complete output Grams, and an analytic bound on both infinite-dimensional complements yields this inequality without assuming the separate global certificate at depth $1.98$. An additional guarded test proves $H_J\succeq10^{-7}A$, hence the unrestricted Schur complement retains at least $10^{-8}$ of the preceding energy. An explicit conditional induction theorem identifies the further data needed for divergent depths and vanishing shifts. The same spatial construction certifies $Q_{0,L_q}\succeq10^{-33}I$ and hence contraction for $0<\omega\le5\times10^{-18}$. At a second quarter-step, a three-interval construction certifies the full-old comparison $H_{J_2}\succeq10^{-8}A_2$, but its residual and absolute positivity tests do not close. Earlier calculations distinguish negative instantaneous generator energy from positive cumulative storage and explain the severe cancellation at a whole-step extension. These are finite-horizon research results; normalization verification, independent review, direct cumulative continuation beyond generator positivity, and an all-depth recursion remain open.
\end{abstract}
\noindent\textbf{Status and dependencies.} The manuscript consolidates five research packets dated 10 September 2026 and the subsequent old-energy comparison and spatial continuation experiments. Version 0.1 and its records remain preserved under \rec{archive/drafts/v0.1_2026-09-10/}. Analytic conclusions use the definitions and finite-horizon framework of Weil-depth v0.3 at repository commit \rec{a566944dc1be2899e37fce3d0e857516ced33d8f} \cite{WeilDepth}. The author's independent normalization verification is pending. New analytic reductions and their implementations require independent mathematical review. ``Certified'' below means a recorded guarded Arb sign computation together with its stated analytic reduction, within these working conventions. It does not mean an independent proof audit, formal verification, or a proof of RH. Authorship and submission metadata remain for the author to finalize.
...
\end{equation}
An additional guarded test, proved in Section~\ref{sec:old-energy-comparison}, gives
\[
 H_J\succeq10^{-7}A,\qquad A-B^*F^{-1}B\succeq10^{-8}A.
\]
...
The factor $0.9$ covers every old input. The earlier, stronger factor $0.004$ covers only the original 128 old modes. The additional comparison proves a retained fraction $10^{-8}$ of $A$ at this first step. It does not prove a uniform retained fraction at arbitrary depth: the carried metric is $H_J$, which can be much smaller than $A$ in weak directions.
...
\]
where $H_k=\sum_{j=1}^k1/j$ and $K_\ell\ge |w_0(0)|+\int_0^\ell|w_0^{\prime}(t)|\,dt$. Write $w_0(t)=(G_0(t)-1)/t$. The gamma cross kernel is $-1/(2s)-w_0(s)/2$. The singular part has norm at most $\pi/2$.
...
The record identifiers below are used throughout this manuscript. Paths for R1--R9 are relative to \rec{archive/drafts/v0.1_2026-09-10/numerics/history/}; full file names, source hashes, and run parameters are provided in the archived \rec{CLAIMS.json} and the packet's verification manifests. All five dated research directories, including earlier diagnostics and unsuccessful attempts, are supplied unchanged. Their chronological notes describe the state at their date of creation; this manuscript and the current \rec{STATUS.md} state the consolidated status.
...
The three inherited compressed matrix files are delivered outside the repository-ready tree: the complete spatial head and two output Grams, and the old-depth caches at 128 and 256 modes. The caches accelerate regeneration; only the spatial archive is needed for the direct relative replay. The tracked \rec{ARCHIVES.md} describes both file and canonical matrix-content hashes, regeneration, and the \rec{STORAGE_DEPTH_ARCHIVES} lookup rule. A matching hash establishes data identity under the documented serialization; the proof still requires arithmetic sign tests. No claim of cross-version bitwise portability is made. The earlier global $1.98$ wrapper rebuilds its matrices; its raw matrices were not retained. Version 0.2 adds a fourth external archive for the three-interval second step, described in Section~\ref{sec:partition-step}. Current records R10--R13 and their exact scopes appear in \rec{CLAIMS.json}; R11 includes unsuccessful sufficient tests.

\begin{center}\small
\begin{tabular}{@{}p{0.07\linewidth}>{\raggedright\arraybackslash}p{0.38\linewidth}>{\raggedright\arraybackslash}p{0.47\linewidth}@{}}\toprule
ID & Version 0.2 result and scope & Current numerical records\\\midrule
R10 & First-step full-old comparison, $\mu=10^{-7}$ & \rec{records/metric-comparison-2048.json} and 4096-bit record\\
R11 & Second-step full-old comparison, $\mu=10^{-8}$; absolute and scalar residual guards unsuccessful & \rec{records/second-quarter-build.json}, 2048-bit and 4096-bit comparison records\\
R12 & Separate-tail residual searches unsuccessful & \rec{records/second-quarter-weighted-1024.json}, near-one and 2048-bit records\\
R13 & Expanded moment, output, and sign-validator consistency checks & \rec{records/spatial-consistency.json}\\\bottomrule
\end{tabular}
\end{center}
Paths in this additional table are relative to the current \rec{numerics/}
directory. The complete filenames and claim coverage are in the current
\rec{CLAIMS.json}. The second-step continuation coefficients form a separate
small exact-rational record. No second-step full-operator positivity or
shift interval is inferred from R11 or R12.

\section{A comparison with the preceding energy}
\label{sec:old-energy-comparison}

The following additional computation uses the original spatial matrix archive
and the original exact rational continuation. It is not an independent
reconstruction of those matrices. All statements remain in the working
Weil-depth v0.3 normalization, with its stated independent-review obligations.

First, the gamma-tail bound used here and in the spatial complement argument
requires the variation hypothesis
\begin{equation}\label{eq:corrected-variation}
 K_\ell\ge |w_0(0)|+\int_0^\ell |w_0'(t)|\,dt,
 \qquad w_0(t)=\frac{G_0(t)-1}{t}.
\end{equation}
Version 0.1 stated only a supremum hypothesis; the variation hypothesis above is the required correction.
Indeed, for causal convolution,
\[
 S_{w_0}=(w_0(0)I+S_{w_0'})I_1,
\]
so the primitive estimate on the polynomial complement and Young's inequality
give the required bound. The historical implementation already computes a
valid choice:
\[
 K_\ell=
 \sum_{j=1}^{d}|g_j|\ell^{j-1}
 +\frac{256}{3}\frac{(\ell/3)^d}{1-\ell/3},\qquad 0<\ell<3.
\]
Termwise integration of the differentiated power series, with the same
Cauchy coefficient majorant for the remainder, proves
\eqref{eq:corrected-variation}. No numerical constant changes under this
correction.

\begin{proposition}[Additional computer-assisted old-energy comparison]
\label{prop:old-energy-comparison}
At
\[
 a=\log7,\qquad h=\tfrac14\log(8/7),\qquad L_q=a+h,
\]
let \(A=Q_{0,a}\), \(F=Q^\gamma_{0,h}\), and let \(B\) be the complete spatial
cross block. Use the original rational \(J_{128,32}\), extended by zero to the
old orthogonal complement. With
\[
 H_J=A+B^*J+J^*B+J^*FJ,
\]
the new archive-based guarded test gives, on the full old form domain,
\begin{equation}\label{eq:full-old-mu}
 H_J\succeq10^{-7}A.
\end{equation}
Combined with the previously established full-domain inequality

\[
 R^*F^{-1}R\preceq\tfrac9{10}H_J,\qquad R=B+FJ,
\]
this implies
\begin{equation}\label{eq:full-old-retention}
 A-B^*F^{-1}B\succeq10^{-8}A,
 \qquad
 \|F^{-1/2}BA^{-1/2}\|^2\le1-10^{-8}.
\end{equation}
The new variable eliminated through \(F^{-1}\) is unrestricted.
\end{proposition}

\begin{proof}[Reduction and numerical premises]
Let \(P_o,P_n\) retain \(N=256,M=32\) old and new modes,
and write \(P=P_o\oplus P_n\), \(K_o=I-P_o\) on the old space.
For the exact compressed quantities set
\[
 H=PQP,\qquad H_o=P_oQP,\qquad
 G_o=(\Pi_o QP)^*(\Pi_o QP),
\]
where \(\Pi_o\) is projection onto the complete old output interval.
Thus
\[
 E_o=G_o-H_o^*H_o=(K_o\Pi_o QP)^*(K_o\Pi_o QP).
\]
All these outputs exist in \(L^2\) on the retained piecewise polynomial inputs.
The notation for \(J\) also denotes its \(M\)-by-\(N\) finite matrix when used
in the following column maps:
\[
 T_o=\begin{pmatrix}I_N\\J\end{pmatrix},\qquad
 U_\mu=\begin{pmatrix}(1-\mu)I_N\\J\end{pmatrix},\qquad 0<\mu<1.
\]
Put \(M_\mu=H_J-\mu A\). Its old-head compression is
\[
 \mathsf H_\mu=T_o^*HT_o-\mu A_N,
 \qquad A_N=P_oAP_o.
\]
Since \(J K_o=0\) and \(K_oJ^*=0\), its old-tail output on the retained
old inputs is
\[
 K_oM_\mu P_o=K_o\Pi_o QP\,U_\mu.
\]
Consequently its exact leakage Gram is
\begin{equation}\label{eq:mu-leakage}
 \mathsf E_\mu=U_\mu^*E_oU_\mu.
\end{equation}
Its tail compression is exactly
\[
 K_oM_\mu K_o=(1-\mu)K_oAK_o.
\]

The corrected local gamma-tail bound and compression of the full arithmetic
norm bound \(\rho\) give
\[
 K_oAK_o\succeq\beta_A I,
 \qquad \beta_A=g_{a,256}-\rho>1.7653403662909112.
\]
For a certified lower bound \(\beta_\mu\le(1-\mu)\beta_A\), with
\(\beta_\mu>0\), the finite sufficient Schur inequality is therefore
\begin{equation}\label{eq:mu-schur}
 \beta_\mu\mathsf H_\mu-\mathsf E_\mu\succ0.
\end{equation}

For clarity, use tildes for the model finite matrices stored in the archive.
The inherited head error is at most \(\eta\). The inherited leakage error
\(e\) also bounds
\(\|E_o-(\widetilde G_o-\widetilde H_o^*\widetilde H_o)\|\):
restricting the output does not increase the output error, and the full-output
norm bound is still an upper bound for the restricted output. The corresponding
head and leakage errors in \eqref{eq:mu-schur} are at most
\[
 \eta(\|T_o\|_F^2+\mu),\qquad e\|U_\mu\|_F^2.
\]
The validator subtracts
\begin{equation}\label{eq:mu-budget}
 \left\{\beta_\mu\eta(\|T_o\|_F^2+\mu)
             +e\|U_\mu\|_F^2\right\}I
\end{equation}
from the model test before interval LDL factorization. A strict positive
finite margin together with the positive tail floor proves coercivity of
\(M_\mu\) by its own Schur complement.

At the exact rational threshold \(\mu=10^{-7}\), tests at 2,048 and 4,096 bits
both pass all 256 pivots. The certified tail floor exceeds
\(1.7653401897568746\), and the subtracted error is below
\(2.641\times10^{-53}\). These are re-evaluations of the saved matrix balls
at the indicated precisions; the original matrix construction used 6,144 bits.
The minimum pivot lower bound is approximately \(2.4638394\times10^{-5}\),
which is a sign diagnostic and is not used as a spectral floor.
The source and full records are supplied as
\texttt{metric\_comparison.py}, \texttt{metric-comparison-2048.json}, and
\texttt{metric-comparison-4096.json}.

This proves \eqref{eq:full-old-mu}. The residual identity gives
\[
 S=H_J-R^*F^{-1}R\succeq\tfrac1{10}H_J\succeq10^{-8}A.
\]
Normalize by the coercive old form \(A\) to obtain the coupling bound in
\eqref{eq:full-old-retention}.
\end{proof}

\section{A conditional induction with explicit shift selection}
\label{sec:conditional-induction}

The next result states sufficient data for an all-depth argument. Its
hypotheses are currently verified only for the finite step specified above;
it is not a proof that the construction continues indefinitely.

\begin{theorem}[Conditional continuation scheme]
\label{thm:conditional-continuation}
Suppose \(L_{j+1}=L_j+h_j\), \(h_j>0\), and \(\sum_jh_j=\infty\).
Assume \(Q_{0,L_0}\succeq m_0I>0\). At every step suppose the spatial form
has the closed block realization
\[
 Q_{0,L_{j+1}}=
 \begin{pmatrix}A_j&B_j^*\\B_j&F_j\end{pmatrix},
 \qquad A_j=Q_{0,L_j},\quad F_j\succeq f_j I>0,
\]
with bounded cross block. Let \(J_j\) be bounded into the operator domain of
\(F_j\), with \(F_jJ_j\) bounded, and define
\[
 H_j=A_j+B_j^*J_j+J_j^*B_j+J_j^*F_jJ_j,
 \qquad R_j=B_j+F_jJ_j.
\]
Suppose the full-domain inequalities
\begin{equation}\label{eq:induction-premises}
 H_j\succeq\mu_jA_j,
 \qquad R_j^*F_j^{-1}R_j\preceq\theta_jH_j,
 \qquad \mu_j>0,\quad0\le\theta_j<1
\end{equation}
are proved for every \(j\). Let \(q_j\ge\|J_j\|\) and
\[
 \tau_j=\frac{\sqrt{q_j^2+4}+q_j}{2},\qquad
 m_{j+1}=\frac{1-\sqrt{\theta_j}}{\tau_j^2}
                 \min\{\mu_jm_j,f_j\}.
\]
Then \(Q_{0,L_j}\succeq m_jI>0\) for every \(j\).

Assume also the working evolution identity and, at every finite \(L_j\),
the estimate
\[
 \|Q_{s,L_j}-Q_{0,L_j}\|\le C_{L_j}s^2,
 \qquad 0\le s\le\tfrac12,\quad C_{L_j}>0.
\]
Choose
\[
 0<\omega_0\le\min\{\tfrac12,\sqrt{m_0/(2C_{L_0})}\},
 \qquad
 \omega_j=\min\left\{\frac{\omega_{j-1}}2,
                       \frac12,\sqrt{\frac{m_j}{2C_{L_j}}}\right\}
 \quad(j\ge1).
\]
Then \(\omega_j\downarrow0\) and
\[
 \|V_{\omega_j,L_j}\|\le\exp(-m_j\omega_j/2)<1.
\]
Thus these hypotheses supply the divergent-depth, vanishing-shift sequence
required by the inherited diagonal criterion.
\end{theorem}

\begin{proof}
For \(g=J_jf+v\), the residual bound gives
\[
 Q_{0,L_{j+1}}[f,g]
 \ge(1-\sqrt{\theta_j})\{H_j[f]+F_j[v]\}
 \ge(1-\sqrt{\theta_j})\min\{\mu_jm_j,f_j\}
                (\|f\|^2+\|v\|^2).
\]
The graph map \((f,v)\mapsto(f,J_jf+v)\) has norm at most \(\tau_j\).
This follows by bounding \(\|J_jf+v\|\le q_j\|f\|+\|v\|\) and computing
the largest singular value of
\(\left(\begin{smallmatrix}1&0\\q_j&1\end{smallmatrix}\right)\).
Consequently
\(\|f\|^2+\|v\|^2\ge\tau_j^{-2}(\|f\|^2+\|g\|^2)\),
which proves the asserted floor inductively. Every displayed floor is
positive at its finite index; no uniform lower bound in \(j\) is assumed.

For \(0\le s\le\omega_j\), the perturbation bound gives
\(Q_{s,L_j}\succeq m_jI/2\). Integrating the evolution identity and taking
the strong zero-shift limit gives
\(\|V_{\omega_j,L_j}\|\le e^{-m_j\omega_j/2}\).
The halving condition implies \(\omega_j\to0\), while the stipulated sum
of depth increments implies \(L_j\to\infty\).
\end{proof}

For the completed quarter-step, the newly available relative state is
\[
 (\mu_0,\theta_0,(1-\theta_0)\mu_0)
       =(10^{-7},0.9,10^{-8}).
\]
Section~\ref{sec:second-result} establishes the old-energy comparison at the next step, but does not establish its residual premise.
The remaining mathematical task is to produce these inequalities indefinitely
on a divergent-depth schedule, or to prove estimates preventing an adaptive
schedule from accumulating at finite depth. The scalar recurrence is
conservative bookkeeping; the complete continuation energy and leakage
matrices should remain part of the computational state.

\input{spatial_recursion.tex}
...
\item Close the residual premise at the second quarter-step. The old-energy comparison already passes at $\mu=10^{-8}$. Enlarge the 32-mode verification spaces on the short slabs and sharpen their joint complement bound. Reuse the stored full-output matrices and retain the recorded failures as sufficient-test diagnostics.
\item Implement the local gamma representation of Section~\ref{sec:partition-step} to remove the global radius-three restriction, and develop a direct positive-shift residual certificate in the bounded Cayley metric, using \eqref{eq:step} to change shift. This is the route to cumulative contraction where instantaneous positivity is unavailable.
...
\end{enumerate}
The conditional theorem supplies explicit scalar and shift bookkeeping once its stepwise premises hold. No theorem here bounds verification dimensions at successive steps, proves the required comparisons indefinitely, proves shift monotonicity, or identifies a unique critical path. A path can approach zero shift at finite depth if available energy degenerates too quickly. The suggested ``edge'' analogy is motivational; no equivalence with the De Bruijn--Newman deformation is used. The concrete state now available for recursion consists of \eqref{eq:mainenergy}, \eqref{eq:full-old-mu}, the complete output Grams, and the exact rational continuation. The conditional theorem explains precisely how sufficiently many further certified states would complete the depth--shift argument.
...
The normalized-coupling identities and residual identity are abstract analytic results. The numerical sign assertions in R1--R9 and the new old-energy comparison are finite computations with explicit error bounds and the indicated domain coverage. Finite-input diagnostics are guides to the construction, not replacements for these domain and error arguments.
