"""Append a second quarter slab, inheriting a certified first spatial head of any slab dimension.

Variant of build_step.py (v0.3): the seed archive is chosen by --seed-relative and its
old/new mode counts are read from the archive. build_step.py itself is left unchanged so
that the recorded second-step archive still verifies against its builder-source hashes.

The default seed is hash-bound to v0.1; --seed-relative (with build_from_seed.py)
extends a freshly built first step with a different slab dimension. All generated matrix data must be
...
def run(args):
    ...
    seed_old,seed_new=seed['old_modes'],seed['new_modes']
    ...
    del seed
    h=(logn(8)-logn(7))/4
    geo=Geometry([logn(7),h,h],[seed_old,seed_new,args.new],args.degree)
    ...
    head=c.sym(head)
    print(f'Head assembled; retaining the first {seed_old+seed_new} modes',flush=True)
    grams=[]
...
if __name__=='__main__':
    ...
    p.add_argument('--seed-relative',default='history/quarter-step-closure-20260910/closure_256_32.matrices.json.gz',
                   help='catalog-relative path of the first-step archive to extend (its old and new mode counts are read from the archive)')
    run(p.parse_args())
