...
For the centered zero extension $f_c$ use $\widehat f_c(\tau)=\int f_c(x)e^{-i\tau x}\,dx$, and put $C(f)=\int f_c(x)\cosh(x/2)\,dx$, $S(f)=\int f_c(x)\sinh(x/2)\,dx$. The central form is
...
\subsection{What finite-horizon positivity at fixed $L$ can see}\label{sec:zeros}
...
\[
 g(t)=e^{t/2}\Bigl(\frac{\sinh t}{t}\Bigr)^{\omega-1},\qquad y_c(t)=\omega\int_0^1v^{\omega-1}e^{ct(1-v)}g(tv)\,dv,\qquad
