...
- [Critical path](investigations/critical-path/README.md): adaptive EMA smoothing and cumulative positivity; a completed L=1/2 audit and full-output pilot, corrected limiting lemmas, 57 inherited controls, and an explicit omitted-input obstruction.
...
| Investigation | Scope and status |
|---|---|
| [Critical path](investigations/critical-path/README.md) | Adaptive-EMA audit and L=1/2 pilot: corrected trace/removal lemmas, a stronger two-sided input-smoothing limit, complete output norm diagnostics, and an explicit omitted-input obstruction. An exact rational contraction bound applies only to one deliberately over-smoothed arithmetic operator; no original-transfer or admissible all-depth certificate. The 57 inherited controls and v0.4 baseline are preserved. |
