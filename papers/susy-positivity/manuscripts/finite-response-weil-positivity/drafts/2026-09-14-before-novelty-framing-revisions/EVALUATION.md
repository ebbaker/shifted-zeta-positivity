...
Revision note, 14 September 2026: mathematical and framing changes following Claude's additional review are documented in [the latest response](reviews/response_claude_20260914.md). The conjecture now retains the admissible cutoff as a parameter; the draft states its present dimension growth and distinguishes exploratory spectra from certificates. The main paper is 7 pages and the expanded derivations are 19 pages. The [first response](reviews/response_20260913.md) and both earlier versions remain available. This remains an AI-assisted assessment awaiting human review.
...
For each positive integer n, work on an interval of length n. All operators are defined using the gamma multiplier, the exact contact, both pole terms, and every active prime power. Let N be any integer satisfying b((N*pi/n)^2) > beta_n + 1/16. Such cutoffs exist unconditionally. Let P select the first N cosine modes and Q the rest. Set

\[
A_{n,N}=P W_nP,\qquad B_{n,N}=QW_nP,\qquad H_{n,N}=QW_nQ.
\]

Here H_{n,N} denotes the operator associated with the restricted high form, on its specified domain; the high space need not be invariant under W_n. The high-sector positivity theorem proves its invertibility without assuming RH. Define

\[
S_{n,N}=A_{n,N}-B_{n,N}^*H_{n,N}^{-1}B_{n,N}.
\]

**Finite-response conjecture: S_{n,N} is positive semidefinite for every integer n >= 1 and every admissible N.** Equivalently, one admissible N at each n suffices. The certified search is an optional implementation; no numerical routine is needed to define this family.

The interpretation is that the high-mode response cannot subtract more energy from a low-mode input than its original low-mode energy. Completing a square proves that S_{n,N} >= 0 if and only if the complete Weil form is nonnegative on that interval. All integer interval lengths cover every compact support. The classical Weil criterion then gives both implications of the RH equivalence. For the alternative notation below, fix one admissible N at each n and abbreviate the response by S_n.
...
The smallest cutoff allowed by the present bound grows doubly exponentially: log log N_min(L) = L/2 + O(log L). This is a limitation of this particular sufficient bound. The companion now includes exploratory length-one upper-response eigenvalues and an image expansion whose remainder decreases exponentially at fixed length. The numerical example has a smallest eigenvalue near 10^(-6); it supplies no certified sign for the exact Schur response. The N=4 rational result remains solely a high-sector certificate. A complete worked arithmetic lower enclosure would strengthen the computational illustration, but would not prove RH or automatically establish a new positivity interval. No rate of decay of the smallest exact eigenvalue in n, uniform precision requirement, or practical complexity bound is inferred.
...
During the initial evaluation, all ten existing exact-algebra programs reproduced their saved output: **59 checks passed**. That package validator checked 130 file hashes, 231 local links, four build records and three draft snapshots. These historical checks establish reproducibility of finite algebra and file identity; they do not verify Hodge theory, operator domains, compactness, infinite limits or the remaining arithmetic sign. The current revision's checks are separately recorded in VALIDATION.json and the latest review response.
