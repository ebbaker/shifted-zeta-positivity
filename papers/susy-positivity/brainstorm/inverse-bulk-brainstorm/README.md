...
| Note | Contents |
|---|---|
| [Bottleneck and operator search, 16 Sep 2026](BOTTLENECK_AND_OPERATOR_SEARCH_20260916.md) | A layered account of where the construction problem is actually blocked; a numerical observation that the critical subspace of the compression is bandlimited below the first zero; three OPE-level fingerprints for the source operator; assessment of five proposed directions (fixed-theory operator search, Wilson lines, `L`-families, non-constructive existence, perturbation theory, OPE); a proposed candidate test battery. |
...
parameters from the twisted-trace classification -- is retracted there.

Notes written by a language model name the model at the top.
