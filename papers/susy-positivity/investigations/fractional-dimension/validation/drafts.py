...
CHECKS = {
    "check_dimension_dictionary.py": "dimension-dictionary-checks.json",
    "check_which_dimension.py": "which-dimension-checks.json",
    "check_residue_volume.py": "residue-volume-checks.json",
    "check_epstein_scattering.py": "epstein-scattering-checks.json",
    "check_two_blaschke.py": "two-blaschke-checks.json",
    "check_the_trade.py": "the-trade-checks.json",
}
...
def record_build(args):
    ...
    record = {
        "schema": 1,
        "title": "A scattering matrix without a space",
...
def save_snapshot(args):
    ...
    with tempfile.TemporaryDirectory(prefix="draft-stage-") as temporary:
        ...
        (prepared / "README.md").write_text(
            ...
            "Historical snapshot of *A scattering matrix without a space*.\n\n"
