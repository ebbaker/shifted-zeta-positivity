...
to~\ref{sec:cayley} are short and elementary once the objects are in
...
$V_{\omega,L}$ in an orthonormal sine basis from the kernel alone and measures
its contraction data. The assembly is more elementary than it first looks:
written in each atom's own coordinate the kernel is $\tau^{\omega-1}$ times a
function holomorphic on $|\tau|<\pi$, so the spike that carries half its mass
below $\tau=2^{-1/\omega}$ belongs in a Gauss--Jacobi \emph{weight} and is never
evaluated at all. Nothing then underflows, thirty nodes per atom suffice, the
whole computation runs in double precision, and one atom has an exact Laplace
transform built from $\Gamma$ alone against which it can be certified.

What the instrument says: the compression is a strict contraction at every
horizon and shift computed, and the smallest eigenvalue of its defect satisfies
$\lambda_{\min}(D_{\omega,L})/2\omega\to m_L$, the margin of the Weil form in the
same basis, at $L=\log3$, $\log5$ and $\log7$ --- margins of
$6\times10^{-8}$, $2.5\times10^{-17}$ and $2.1\times10^{-22}$. The transfer side and
the form side of the program, assembled from different presentations of the
explicit formula, agree at first order over fourteen orders of magnitude. The
deviations are then accounted for exactly. On the exact compression the defect
expands as $D_{\omega,L}=2\omega Q_L-\omega^2(2Q_L^2+[Q_L,A_L])+O(\omega^3)$,
...
\eqref{eq:intro-firstorder} at the minimizer is $-\omega m_L^2$. What is seen in
a finite basis instead is a truncation term, the energy of $A_Lf$ outside the
basis, linear in $\omega$ with a coefficient measured at each horizon, which
under nesting shrinks and overshoots exactly as the theory says it must. The
mass of the assembled kernel beyond the horizon has, to first order in $\omega$,
a closed form from the logarithmic derivative of $\xi$ alone --- poles, digamma
and primes, no zeros --- and the assembly matches it at all three horizons.

Section~\ref{sec:cayley} then does two things the instrument makes possible.
It computes the Cayley coordinate $P=\frac2\omega\re(I-V)(I+V)^{-1}$, in which
contraction \emph{is} positivity, verifies its congruence with the defect to
working precision, and shows it is an even function of the shift --- so the
first-order law holds there with no first-order correction at all, for the
one-line reason that the transfer inverts under $\omega\mapsto-\omega$ while
compression respects products. And it puts the transfer's margin beside the best
zero-free surrogate for it: the phase-matched unfolded lattice, which reproduces
the density of the zeros but misses the margin by factors between $0.76$ and
$2.19$ at the three horizons where the transfer, containing no zero and no
lattice, misses by $0.54\omega$, $1.20\omega$ and $1.85\omega$.
...
results at three horizons. Section~\ref{sec:cayley} computes the Cayley
coordinate and compares the transfer with the lattice controls.
Section~\ref{sec:outlook} ranks what remains. Appendix~\ref{app:numerics}
