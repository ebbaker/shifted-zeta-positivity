...
program and record that produced it, together with the page count, the engine
...
check program or preserved record changes after the record is written,
...
\[`../../background.pdf`\] and the Wilson-lines companion manuscript for results
quoted by number, but compiles without them. `CHECKS` in `validation/drafts.py` registers
`numerics/check_contraction_margin.py` against
`numerics/records/contraction-margin-checks.json`, so `check --replay` re-runs it
and requires byte-identical output; the exploratory programmes and their records
live in `numerics/exploratory/` and are described in the manuscript's
Appendix A.
