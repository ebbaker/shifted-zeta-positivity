...
quoted by number, but compiles without them. This investigation has no
registered check programmes yet, so `CHECKS` in `validation/drafts.py` is empty
and `check --replay` replays nothing; the exploratory programmes and their
records live in `numerics/exploratory/` and are described in the manuscript's
Appendix A.
