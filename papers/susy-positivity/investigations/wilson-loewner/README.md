...
YM continuation, 25 September: [electric parity and the archimedean response](YM/notes/ELECTRIC_PARITY_ARCHIMEDEAN_RESPONSE_AND_PHASE_OBSTRUCTION_20260925.md) derives a complete signed mixed Weil limit in the fixed four-dimensional slab, including the gamma contact and logarithmic growth. The [character/current audit](YM/reviews/CHARACTER_CURRENT_AND_PRIME_DOMAIN_AUDIT_20260925.md) rederives the prime identity and proves stronger phase and positive-prime domain obstructions. A positive OS source realization remains open; see the maintained [YM outline](YM/PROJECT_OUTLINE.md).

The [YM investigation](YM/README.md) now has an exact [finite-slab reflection-boundary construction](YM/notes/FINITE_SLAB_REFLECTION_AND_DISCRETE_HIERARCHY_20260924.md) and a [first interacting four-dimensional SU(2) test](YM/notes/FIRST_INTERACTING_SLAB_TEST_AND_CONTINUATION_20260924.md). Boundary-only loop multiplication preserves the reflected null space; discrete projection has explicit Schur residuals and a forward/backward output bound. The measured small-family residuals remain large, and adding curvature observables can worsen a later Wilson estimate. The next physics tests therefore target local plaquette sectors and slice-only smoothing. The [opening analysis](YM/notes/YM_POSITIVE_HIERARCHY_PRELIMINARY_ANALYSIS_20260924.md) and [plan](YM/notes/YM_REFLECTION_AND_HIERARCHY_RESEARCH_PLAN_20260924.md) remain as the continuous regulated starting point. No vacuum-limit, continuum or arithmetic realization is established.

The [updated assessment of reflection positivity and observable hierarchies](notes/UPDATED_PATH_ASSESSMENT_REFLECTION_AND_HIERARCHIES_20260924.md) reviews WZW, all three N4SYM sessions and the second arithmetic-storage session at commit `488c782` (24 September 2026). Its arithmetic priorities are the actual semilocal Sonin residual and a focused canonical-system comparison; its physics priority is a positive-norm treatment of the YM/N4SYM insertion hierarchy. It supersedes the earlier recommendation to pursue a chain of finite append certificates. No all-window positivity mechanism or exact physical arithmetic realization has been established.

The [second arithmetic-storage session](arithmetic-storage/notes/FIRST_PRIME_JOIN_EQUIVALENCE_AND_PRIME_WEIGHT_RIGIDITY_20260924.md), committed in `488c782`, closes the first-prime join using the existing weil-depth pipeline. It identifies the append hypothesis with joined-window positivity, gives a research-note obstruction to a fixed margin for joins of bounded-below length, and certifies a narrow admissible range for the first prime's weight. Specialist review remains outstanding. See the [review](arithmetic-storage/reviews/review_claude_first_prime_session_20260924.md) and [continuation](arithmetic-storage/notes/RESEARCH_CONTINUATION_AFTER_FIRST_PRIME_CLOSURE_20260924.md). The [first-session analysis](arithmetic-storage/notes/FIRST_PRIME_CONTINUATION_ANALYSIS_20260924.md) and [opening program](arithmetic-storage/notes/ARITHMETIC_STORAGE_PROGRAM_AND_GOALS_20260924.md) retain their historical targets; the all-input first-prime task they left open is now internally certified.

The [original lessons](notes/LESSONS_FROM_WZW_FOR_YM_AND_N4SYM_20260923.md) led to the [N4SYM proposal](N4SYM/notes/N4SYM_DISPLACEMENT_LOEWNER_RESEARCH_PROPOSAL_20260923.md). The subsequent [N4SYM work](N4SYM/README.md) supplies a nontrivial flipped-return Wilson observable, two exact smooth-field evolution equations, a near-BPS corner analysis, and an explicit Schwinger–Dyson nonclosure inventory. The finite-mass classical-string calculation restores positive finite-time work accounting by retaining the dressed-source state; its linear memory remains a single exponential. Quartic defect input suggests nonlinear memory, but the complete cubic retarded kernel has not been calculated. The seven programs contain 472 diagnostics. See the [program assessment](N4SYM/reviews/PROGRAM_ASSESSMENT_CODEX_20260924.md) for approximation limits and the prior arbitrary-motion dilaton literature; the absence of temporal broadening should not be presented as a new correction to the 1999 calculation. These results strengthen the physics program without providing the target arithmetic response.
