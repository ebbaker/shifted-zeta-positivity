...
The four standard-library programs contain 299 diagnostics. Replay checks
...
## Additional version-0.5 localization controls

The original four-program replay remains 299 cases. The three additional
controls have their own formats and are included, with records, in the
version-0.5 standalone snapshot. Replay them separately:

    python3 -B numerics/check_localization_hemisphere.py --output /tmp/hemisphere-replay.json
    python3 -B numerics/check_spatial_radial_descent.py --output /tmp/radial-descent-replay.json
    python3 -B numerics/check_hodge_radial_channel.py --output /tmp/hodge-channel-replay.json
