...
a separate internally certified fixed-window anchor and a quantitative
obstruction to one coupling estimate. No successful all-depth recursion
is established.
...
\subsection{The arithmetic EMA anchor and the unresolved append}
...
bounds on the full coupling norm. No all-input upper bound below
one has been obtained.

The next estimate must preserve the directional defect energy
at the join and control both omitted input spaces. Endpoint memory
cannot be reset when windows are adjoined. An operator lower bound
for an instantaneous form also cannot be exponentiated through a
nonnormal flow without justification; the cumulative balance must
retain the actual transfer. Auxiliary smoothing loss and averaging
variance contribute no arithmetic positivity to these results.
