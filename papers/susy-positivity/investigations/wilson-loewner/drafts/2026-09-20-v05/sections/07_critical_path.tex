...
\subsection{A fixed-window foothold and the next coupling estimate}
...
floor transfers to both defect orientations. Nevertheless the
scalar sufficient coupling estimate loses a factor between 55 and
72. Finite complete-output diagnostics near $0.802$ leave the
all-input upper bound unresolved \cite{CumulativeAppend}.
The distinction is detailed in \cite[Section~S9.5]{Supplement}.
The next mathematical step is an energy-weighted bound for the
mixed block and both input complements, carrying endpoint memory
through the join. The localization calculation in
Section~\ref{subsec:main-localized-control} supplies no additional
positivity for this estimate.
