...
\subsection{The later arithmetic matching problem}

The fixed-theory question starts with a driver, its trace and a specified
Wilson observable. Section~\ref{sec:growing-trace} derives its first
quantum equations. If an appropriate boundary construction is found,
the following stronger matching requirement can then be tested.
...
For this later matching problem, the arithmetic differential equation
is intended to \emph{select} admissible
...
For a fixed normalized quantum measure,
$\partial_t\vev{\mathcal O_t}=\vev{\partial_t\mathcal O_t}$
whenever differentiation is justified. Holding a field configuration
fixed while deriving \eqref{eq:variation-summary} is the intermediate
functional calculation, not a replacement of the quantum average.
If an action or an external normalization is varied, its connected
insertion or normalization derivative must also be included.
