...
**Latest update, 24 September 2026 (after commit `488c782`):** the [updated assessment](UPDATED_PATH_ASSESSMENT_REFLECTION_AND_HIERARCHIES_20260924.md) supersedes the append ranking below, including the earlier status correction. The first-prime join is internally closed. At zero shift its coupling hypothesis is exactly a relative joined-window positivity inequality; an absolute central floor can imply the needed bound. Thus the earlier phrase “a useful control, not a solution” does not distinguish two independent positivity tasks. What remains missing is a continuation mechanism. Priority now goes to the actual Sonin residual, a focused canonical-system estimate, and positive-norm control of the physical observable hierarchy. The original assessment and prior correction remain as history.
