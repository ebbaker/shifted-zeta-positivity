...
**Pause and manuscript consolidation, 25 September 2026.** At the author's request, the investigation is paused and all completed results are consolidated in the [35-page draft](manuscript.pdf), with [editable source](manuscript.tex). Its theorem statements separate proved deductions from unverified physical occurrence hypotheses and numerical diagnostics. The [consolidation audit](reviews/MANUSCRIPT_CONSOLIDATION_AUDIT_20260925.md) records the residual-formula typographical repair and verification scope; [DRAFT_HISTOR.md](DRAFT_HISTOR.md) indexes this live manuscript. The historical priorities below remain background, not an active continuation request.
...
Status: working research outline. The full OS axioms, continuum YM existence, and a physical mass gap may be assumed for explicitly stated conditional arguments; further natural physical assumptions must be identified. The arithmetic realization is an open objective. No RH proof is claimed.
...
Continuation, 25 September 2026: the [independent review](reviews/OFF_DIAGONAL_AND_GLOBAL_MECHANISMS_REVIEW_20260925.md) and [source-domain controls](notes/SOURCE_DOMAIN_AND_WEIGHT_TRANSPORT_REVIEW_CONTROLS_20260925.md) clarify the admissible completion and why Hilbert-space membership alone does not identify physical sources. The [new winding-source calculation](notes/NATIVE_WINDING_MELLIN_SOURCES_AND_ELECTRIC_PAIRING_20260925.md) now specifies a global law in the fixed interacting SU(2) slab, proves its electric insertion is well defined after smearing, and derives its exact mixed OS pairing. It also proves that this law fails the Weil target because it is locally bounded in the ordinary input L² norm. Native winding Euler and twisted Poisson identities do not repair that metric mismatch. The note gives the precise prime-separation singularities required of the next source law; these new deductions await independent review.

Character continuation, 25 September 2026: the [new character-channel calculation](notes/CHARACTER_CHANNEL_LIMITS_AND_PRIME_MIXED_PAIRINGS_20260925.md) obtains the exact limiting mixed identity \(\langle S_R f,V_aS_R g\rangle_\nu\to a^{-1/2}\langle f,U_{\log a}g\rangle\) in the fixed actual state. Its operator Euler coefficients yield the prime part of the Weil form through a distributional insertion, with controlled prime-cutoff tails. Rational phase branching accounts for the remaining norm, and a full electric–Wilson current approaches arithmetic differentiation on the identity component. The packets lose their norm in the weak physical limit; prime squared differences introduce a divergent local term; several winding-created components have divergent current norm. The immediate task is an independently derived archimedean completion compatible with positivity and physical source occurrence. Ordinary two-dimensional handle/cylinder gluing supplies only local packet multipliers in the comparison tested here. These new deductions also await independent review.

Archimedean continuation, 25 September 2026: the [character/current/domain audit](reviews/CHARACTER_CURRENT_AND_PRIME_DOMAIN_AUDIT_20260925.md) rederives the preceding formulas, clarifies that the infinite prime insertion is not an OS Hilbert source, and proves an opposite current at phase pi and a zero-domain theorem for the unrenormalized positive prime sum. The [new electric-parity response](notes/ELECTRIC_PARITY_ARCHIMEDEAN_RESPONSE_AND_PHASE_OBSTRUCTION_20260925.md) specifies a normalized derivative of the compressed, density-compensated radial electric operator and a logarithmic electric/holonomy-angle insertion in the same state. Its mixed packet limit gives exactly \(\operatorname{Re}\psi(1/4+i\tau/2)-\log\pi\), with the local normalization and logarithmic growth derived through cosine/Mellin analysis. Combined with the original finite winding insertions, this gives the complete **signed** Weil mixed limit on the global pole-neutral class, in a stated iterated limit. It is not a norm realization: the response diverges on nonidentity winding phases, fixed closable insertions cannot recover a nonzero strong source limit from these weakly escaping packets, and no positive prime renormalization has been established. Nineteen additional floating controls are diagnostics. These proofs remain model-assisted research deductions awaiting independent review.

Critical review, 25 September 2026: the [electric-parity assessment](reviews/ELECTRIC_PARITY_RESPONSE_CRITICAL_REVIEW_20260925.md) rederives the latest identities and retains the complete **signed** mixed limit; no central formula is retracted. Its [bounded winding-occurrence theorem](notes/BOUNDED_WINDING_OCCURRENCE_OBSTRUCTION_20260925.md) proves that no nonzero bounded forward intertwiner recovers the limiting character winding module inside the original one-plaquette class space, even on the identity-phase winding orbit. Consequently bounded scale-dependent recovery preserving all windings is excluded. The recommended priority is an independently positive physical source pairing with appropriately weaker source relations, not further gamma matching or phase repair alone. General smooth-test source laws in the full fixed YM space remain open.

OS/mass-gap continuation, 25 September 2026: the [new source calculation](notes/NATIVE_RECENTERING_AND_GAPPED_POSITIVE_COMPLETION_20260925.md) uses the full electric–Wilson current at time R to recover identity-phase packets strongly in the original finite-state Hilbert space. Their exact norm is ordinary L2, and the additional winding components have an explicit weak-limit norm defect. A new global domain theorem excludes completing an L2-isometric source by one fixed closable operator to obtain the Weil norm. A positive constrained-energy/Schur prescription has a divergent prime limit whenever it retains a uniform coercive bound. For an assumed continuum YM vacuum, local phase-space compactness supplies compact sets of actual bounded local preparations, but not their arithmetic finite feasibility. The [assumption audit](reviews/OS_MASS_GAP_AND_SOURCE_DOMAIN_AUDIT_20260925.md) records the separation between the fixed regulator and these allowed continuum hypotheses.
...
The conditional objective explicitly includes the possibility that existence and a mass gap for four-dimensional YM, possibly with additional natural physical assumptions, imply RH. OS reconstruction, a spectral gap, and local phase-space compactness may be used in that investigation with their roles stated. An unproved arithmetic source identity or positivity of the completed Weil form is not to be relabeled as a generic physical axiom. No logical nonimplication from the physical axioms is claimed either.
...
| Independent YM source module with the required arithmetic relations | Main open existence problem; one explicit winding/insertion law has been constructed and excluded |
...
| Critical Mellin winding source with electric insertion | Defines a genuine fixed-state generalized law, proves its Abel/form-domain limit, and derives the exact interacting mixed pairing; an all-input estimate excludes the Weil match | A different independent source law passing logarithmic growth and the separated prime identity |
| Native winding Euler/Poisson controls | Derives prime coefficients algebraically and a holonomy-twisted completion identity; identifies unbounded winding operations and the trivial-holonomy evaluation limitation | A physical mixed pairing, beyond scalar symbols or operator identities in a different norm |
| Prime-separation distribution test | Makes the required off-diagonal fourth derivatives of delta distributions explicit after pole removal | YM observables producing those singularities with the exact coefficients |
| Character packets and weighted winding | Rederived actual adjoints, exact rational phase branching, and distributional prime-power mixed identity | Positive infinite squared differences have domain only zero on the entire class space; no nonzero strong packet limit |
| Electric–Wilson current | A full physical commutator approaches arithmetic differentiation and unitary translation on identity-phase packets | Phase pi reverses the derivative; other roots cause divergent norms; no nonconstant smooth radial current controls every rational phase |
| Normalized electric parity and logarithmic angle response | Derives the complete archimedean multiplier, its contact and high-frequency growth; with finite winding insertions gives the full signed mixed Weil limit | Uses an explicit radial compression/compensation, not the full electric operator; winding phases diverge, positivity and physical source occurrence remain open |
| Native long-time electric recentering | Produces a nonzero strong physical source limit with exact ordinary-L2 pairing and explicit winding norm defect | Bare pairing is not Q; the full winding module is not recovered strongly |
| Global L2 closability obstruction | No fixed closable operator can complete an L2-isometric source to the global Weil norm, even in the positive case | Direct generalized smooth-test source laws remain allowed |
| Coercive positive prime Schur completion | Finite constrained energies are actual positive source norms; their global prime limit diverges at every nonzero fixed bounded readout | Different independently defined constraints or observables need a new positivity/domain argument |
| Continuum local phase-space compactness criterion | Under an explicit natural compactness assumption, finite compatibility can retain bounded local observable preparations after Euclidean damping | Independent arithmetic finite feasibility and preparation bounds remain unproved |
| Universal detecting probe | A single translated response is bounded/tempered iff RH; its growth exponent detects off-line zeros | A physical interpretation giving that bound is not established |
| Full electric-current spectrum | Computes the actual full-state current as a line translation with transverse multiplicity; excludes all finite-vector realizations of the detecting response | Other independently specified source actions remain open |
| Sharp prime-cutoff boundary response | Derives both moving profiles and an actual inserted-vector norm lower bound of order square root of A, using PNT | A nonlocal completion or different regulator needs a separate physical and positivity argument |
| One-correlation occurrence theorem | An actual stationary detecting correlation yields the whole source law by spectral filtering, with domain and continuity proved | The actual YM correlation is the unresolved hypothesis |
| Two-dimensional YM gluing comparison | Gives explicit handle and cylinder packet limits and a logarithmic dimension operator | These standard insertions give local multipliers, not the missing gamma kernel; no model change is adopted |

The earlier analytic entries are proved or scoped in the [global source and Ward-identity audit](notes/GLOBAL_SOURCE_AND_WARD_IDENTITY_AUDIT_20260924.md) and its [native-generator continuation](notes/NATIVE_TRANSLATION_GENERATORS_AND_GENERALIZED_SOURCES_20260924.md), with a subsequent [critical assessment](reviews/GLOBAL_MECHANISMS_CRITICAL_REVIEW_20260924.md). New proofs are in the [determination/extension note](notes/OFF_DIAGONAL_RIGIDITY_AND_COMPACT_SOURCE_EXISTENCE_20260924.md) and [spectral controls](notes/SOURCE_SPECTRA_MODULAR_AND_LIOUVILLE_CONTROLS_20260924.md). The [winding-source continuation](notes/NATIVE_WINDING_MELLIN_SOURCES_AND_ELECTRIC_PAIRING_20260925.md) adds an explicit law, its exact pairing, the domain proof, and its scoped exclusion. These are model-assisted research deductions, not independently validated results or novelty claims. The first interacting test remains a finite statistical diagnostic.
...
Current priority: establish one independently specified stationary family of actual YM sources with the detecting correlation in the [single-probe theorem](notes/SINGLE_PROBE_OCCURRENCE_AND_PRIME_BOUNDARY_RESPONSE_20260925.md). The theorem then supplies the global smooth-test source and proves its spectral-filtering domain. A separate native arithmetic generator is optional. The complete electric–Wilson current is now excluded as that action, including on the other link degrees of freedom; further phase control alone cannot rescue this candidate.

A second, weaker target is to give the already derived signed response a physical interpretation that independently makes it tempered in its arithmetic parameter. The new criterion would imply RH from that bound, but this would not itself be an OS norm realization. OS spacetime temperedness cannot be transferred to logarithmic character scale without proof. Neither this bound nor the actual stationary correlation has been established.

1. **Specify the actual source relation.** Use the same fixed model/state and an admissible smooth-test domain. Derive the one scalar correlation for all translates, or the earlier separated-support mixed identity. Positivity of target finite Gram matrices, an assigned zero spectrum, or a digamma multiplier is not an independent relation.
2. **Preserve the physical pairing.** The current recentering gives a strong L2 source but cannot carry the detecting correlation. The fixed closable completion and coercive prime Schur prescriptions remain excluded. Any new source limit needs actual preparation estimates or another proved occurrence mechanism.
3. **Handle prime completion in its stated domain.** Positive prime differences have zero infinite domain. The sharp signed cutoff also has moving boundary responses of norm at least order square root of A, even after pole neutrality. A source-local subtraction cannot cancel them. A different nonlocal constraint, regulator, or subtraction must be independently specified and its positive completed limit proved.
4. **Use determination and compactness only where their hypotheses are available.** The one-correlation theorem now complements the earlier three-anchor determination theorem. Compact finite compatibility can preserve actual sources; continuum local phase-space compactness is an allowed additional assumption. Their arithmetic feasibility and preparation bounds remain separate obligations; mass gap alone does not supply them.
5. **Screen native actions before refining them.** The full electric current has decaying matrix coefficients, while the detecting response cannot decay. Any other clustering action has the same obstacle along its clustering parameter. Thermal Liouville sources change the state; the boundary multiplication algebra has trivial modular flow. These exclusions do not rule out general source occurrence.
6. **Retain the separate causal target.** A physical preparation/evolution/readout realization of the shifted-zeta transfer still needs its own exact transfer identity and ordinary energy estimate. Any proposed arithmetic defect or auxiliary theory must be identified as a model change.
...
- [One detecting probe, full-current spectrum, and prime-cutoff boundary response](notes/SINGLE_PROBE_OCCURRENCE_AND_PRIME_BOUNDARY_RESPONSE_20260925.md) is the latest continuation, with its [substantive domain audit](reviews/SINGLE_PROBE_CURRENT_AND_CUTOFF_DOMAIN_AUDIT_20260925.md).
- [Native recentering and gapped positive completion](notes/NATIVE_RECENTERING_AND_GAPPED_POSITIVE_COMPLETION_20260925.md) is the preceding continuation, with its [OS/mass-gap and source-domain audit](reviews/OS_MASS_GAP_AND_SOURCE_DOMAIN_AUDIT_20260925.md).
- [Electric-parity response critical review](reviews/ELECTRIC_PARITY_RESPONSE_CRITICAL_REVIEW_20260925.md) records the preceding assessment and research recommendation, accompanied by the [bounded winding-occurrence obstruction](notes/BOUNDED_WINDING_OCCURRENCE_OBSTRUCTION_20260925.md).
- [Electric parity, archimedean response, and phase obstruction](notes/ELECTRIC_PARITY_ARCHIMEDEAN_RESPONSE_AND_PHASE_OBSTRUCTION_20260925.md) records the earlier continuation: a complete signed mixed limit, its independently specified radial insertion and normalization, and explicit phase, positivity, and source-occurrence obstructions.
- [Character/current/prime-domain audit](reviews/CHARACTER_CURRENT_AND_PRIME_DOMAIN_AUDIT_20260925.md) rederives the earlier limits, clarifies distributional admissibility, supplies the opposite pi-current and proves the positive prime form's zero-domain theorem.
- [Character channels and prime mixed pairings](notes/CHARACTER_CHANNEL_LIMITS_AND_PRIME_MIXED_PAIRINGS_20260925.md) is the preceding continuation: actual weighted adjoints, phase branching, a prime-power mixed identity, a native current limit, physical norm escape, the divergent positive contact term, and a scoped two-dimensional gluing comparison.
- [Topology, geometry, and observable options](notes/TOPOLOGY_GEOMETRY_AND_CHARACTER_OBSERVABLE_OPTIONS_20260925.md) motivates the character operation and distinguishes the comparison theories from the current model.
- [Native winding sources and their electric mixed pairing](notes/NATIVE_WINDING_MELLIN_SOURCES_AND_ELECTRIC_PAIRING_20260925.md) gives the preceding explicit global source, exact interacting pairing, domain proof, scoped exclusion, and necessary kernel test.
- [Independent review of the three global-mechanism notes](reviews/OFF_DIAGONAL_AND_GLOBAL_MECHANISMS_REVIEW_20260925.md) records the earlier review, with [source-domain and weight-transport controls](notes/SOURCE_DOMAIN_AND_WEIGHT_TRANSPORT_REVIEW_CONTROLS_20260925.md).
- [Critical global-mechanisms assessment](reviews/GLOBAL_MECHANISMS_CRITICAL_REVIEW_20260924.md) records the preceding review and ranked research recommendation.
