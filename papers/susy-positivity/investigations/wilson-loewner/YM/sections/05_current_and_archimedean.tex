...
The packet limit \eqref{eq:conductor-form} is the classical even
conductor form of Burnol, not an identification of the compressed
operator on $(0,\pi)$ with a global conductor operator.
Adding $B^*M_hB$ for smooth real h changes the limit by
$h(0)\ip f g_2$, by the same cosine localization. Thus the
calculated contact is exact for the specified insertion, while
a physical selection principle for it remains missing.
Both the compensated limit and its contact calculation are universal
over the smooth positive marginals used here.
