...
The exclusion concerns ordinary input norms. A distributional law
such as $Jf=\int f'(x)A(x)\dd x$ can have logarithmic growth even
when $A(x)$ is bounded in Hilbert norm; the covariance's derivatives
need not be bounded. The finite link-function Hilbert space is
infinite dimensional. Neither finite lattice size nor a smooth
positive weight alone excludes all such sources.
...
The normalized prime mixed limit and compensated archimedean limit
hold for every smooth positive marginal in the stated class, and
have not identified a specifically four-dimensional mechanism.
The wound pairings and actual source formulas retain state dependence.
Section~\ref{sec:distributional} additionally derives a compact-return
obstruction for arbitrary exact source laws and tests two specified
distributional physical mechanisms.
Section~\ref{sec:full-electric-preparation} tests two further mechanisms
in the fixed state: full-electric Sobolev point sources and distributional
strict-half-slab preparation. The latter has a generator-free exclusion
and yields necessary bounds for compact-source preparation.
