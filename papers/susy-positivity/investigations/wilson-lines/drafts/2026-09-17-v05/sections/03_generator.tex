...
and that the whole $\omega$-dependence is one scalar factor. Section~\ref{sec:allpass}
then identifies the transfer, and everything after that is a consequence.
...
poles at $p=\pm\frac12\pm\omega$ --- poles of the individual terms, which cancel
in the sum, as Remark~\ref{rem:trap} records --- all to the left of
$\re p=\eta$ since
...
part on the imaginary axis. Remark~\ref{rem:trap} gives the general form of the
phenomenon: the contour may be moved for $K_\omega$ and may not for $a_\omega$.
\end{remark}
