...
Proposition~\ref{prop:unimodular} says why in one word. An all-pass filter
changes only phase, and a phase is a scalar; a one-parameter family of them can
carry no ordering. The proof below is independent of that section and uses only
causality, which is worth having because it locates the property in the
truncation rather than in the arithmetic --- but the reader who wants the reason
rather than the proof should take it from Section~\ref{sec:allpass}.
