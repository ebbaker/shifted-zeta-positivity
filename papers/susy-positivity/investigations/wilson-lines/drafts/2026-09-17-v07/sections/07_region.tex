...
$m_L\approx\varepsilon_L^2Q[Xe_L]$.

What kind of object $m_L$ is, and what its collapse is caused by, is
Section~\ref{sec:sampling}. The short answer, stated here because it changes how
this subsection should be read: $m_L$ is the lower sampling constant of the zeros
on a Paley--Wiener space, it is strictly positive
(Proposition~\ref{prop:positive}), and its collapse is produced by the
\emph{density} of the zeros rather than by their arithmetic
(\S\ref{subsec:density}). The zero-placement precision is therefore a correct
description of the mechanism and a misleading place to look for one.
