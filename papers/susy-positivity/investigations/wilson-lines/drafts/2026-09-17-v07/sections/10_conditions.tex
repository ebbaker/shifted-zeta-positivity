...
transport --- not a character. Its spectral kernel must be a \emph{ratio}, not a
correlator: a unimodular function on the critical line, inner in a half-plane.
Its dilatation generator must \emph{not} be bounded below. And its group delay
must match the expansion
\[
 2\theta'(\tau)=\log\tau-\log2\pi-\frac1{24\tau^2}+O(\tau^{-4})
\]
coefficient by coefficient: the leading one is the \emph{degree}, the $\tau^{-2}$
one fixes the archimedean \emph{shift}, and the constant is the \emph{conductor}.
\end{condition}

The last clause is a normalization check and should be read as one. By
Proposition~\ref{prop:dictfree} it is dictionary-free and by
Theorem~\ref{thm:onefactor} it is, for a single $\Gamma$-factor, a complete
classification --- so it is cheap, sharp, and a candidate must not fail it. But
Remark~\ref{rem:modular} shows it measures the archimedean factor on the critical
line and nothing else: an amplitude with no relation to the zeros passes it
exactly, and the modular surface, whose resonances \emph{are} the zeros, fails it
by a factor of four. It is therefore a filter, not a target to aim a construction
at. The arithmetic content of the programme is in the bounded fluctuation, to
which every clause here is blind.

Section~\ref{sec:delay} is this condition, derived and run; the three middle
clauses replace the single demand "the delay grows like $\log\tau$" of earlier
versions, which Proposition~\ref{prop:nowinding} shows is failed by every
correlator and \eqref{eq:freewinding} shows is passed by a free Hamiltonian.

The first clause names a class and the rest are the test. The exclusion behind
...
\paragraph{And $L$ is a modulus, not a length.} In the ray coordinate the
interval $I_L$ is the annulus $e^{-L/2}<r<e^{L/2}$, of ratio $e^L$ and conformal
modulus $L/2\pi$. Dilations move it and preserve $L$, and nothing else about it is
conformally meaningful, so the program's second parameter is exactly the one
conformal invariant of the configuration. This is the consistency check the
reading below needs: had $L$ been a scale, the dilation invariance of the form
would have been in conflict with the whole $L$-dependence.
