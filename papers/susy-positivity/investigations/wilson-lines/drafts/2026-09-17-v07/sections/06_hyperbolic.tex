...
Equation \eqref{eq:hyperbolic} is (1.10) of \cite{Background}, and it was
obtained independently, with the coordinate centred at the interval's midpoint
and with the congruence
$Q_{\omega,L}=C_\omega(Q-T_\omega QT_\omega)C_\omega$ and the exact criterion
$\lVert\tanh(\omega X)\rVert_{Q\to Q}\leq1$, in the shifted-zeta storage-depth
investigation \cite{CriticalPath}, which has priority. The proof above derives it
from the generator rather than from a kernel expansion; the two agree. That
investigation's $\kappa_L=\lVert X\rVert_{Q\to Q}$ is analysed in
Section~\ref{sec:region}.
\end{remark}

The derivation from the generator is what makes the next remark available.
...
infinite-dimensional label. This is Condition~\ref{cond:bilinear} below.
\end{remark}
