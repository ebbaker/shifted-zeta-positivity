...
def main():
    ...
    SURVEY = [
        ...
        ("modular surface, Gamma part",             [(1.0, 0.5, 0.5)],                      False),
        ("JT / Liouville-QM wall",                  [(1.0, 1.0, 2.0)],                      False),
        ("H_3^+ (m-basis), t = 3",                  [(-1.0, 1.0, 2.0/3)],                   False),
    ...
    SURVEY_FOR_FIT = [r for r in SURVEY if r[0] in (
        ...
        "bulk Liouville, b = 1", "modular surface, Gamma part",
    ...
    # H7: the two survivors ARE Legendre duplication.
    #     [Gamma(1+2 i t)/Gamma(1+i t)]^(1/2) = 2^(i t) pi^(-1/4) Gamma(1/2 + i t)^(1/2)
    for t in (1.3, 7.0, 21.0):
