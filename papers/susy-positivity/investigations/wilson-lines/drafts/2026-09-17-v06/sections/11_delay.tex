...
\[
 ...
 +\sum_{m\geq1}\frac{\Sigma_{2m}}{\tau^{2m}},
 \qquad \Lambda=2\sum_jn_j\beta_j,\qquad
...
coefficient is $\Lambda/\kappa$ and the $\tau^{-2m}$ coefficient is
$\kappa^{2m-1}\Sigma_{2m}$; imposing $\kappa=\Lambda$ gives the claim. For the
...
factor, $n=1$, $a\in\{\frac14,\frac34\}$: the archimedean factor of a degree-one
$L$-function, of either parity, and nothing else.
\end{proposition}
...
Two hypotheses here are not decoration. The common shift is not removable: with
$a_1=\frac12$, $a_2=1$, $n_1=n_2=1$, $\beta_1=0.1576708$, $\beta_2=0.3423292$ both
$\Lambda=1$ and $\Sigma_2=-\frac1{24}$ hold, a positive $B_2$ paying for a
negative one. Nor is integrality: $n=\pm\frac12$ at $a=\frac12$ is the third
solution of Theorem~\ref{thm:onefactor}, and it is the one the $c=1$ matrix model
realizes (\S\ref{ssec:survey}). What is unconditional, for one factor, is
Theorem~\ref{thm:onefactor}.
...
exactly two points of that family, $P=\frac14$ and $P=\frac12$, and both are the
target after Legendre duplication: at $P=\frac12$,
$\big[\Gamma(1+2i\tau)/\Gamma(1+i\tau)\big]^{1/2}
 =2^{i\tau}\pi^{-1/4}\Gamma(\frac12+i\tau)^{1/2}$, which is
...
and it is two lines of arithmetic per candidate. Each row below reads
$(n_j,a_j,\beta_j)$ off the amplitude \emph{as quoted in the literature} --- the
$\Gamma$-structure of every row is a reading, not a re-derivation here --- and
then evaluates $\Lambda$ and the first three invariants.

\begin{center}\small
\begin{tabular}{@{}lrrrl@{}}
\toprule
amplitude & $\Lambda$ & $I_2$ & $I_4$ & verdict\\
\midrule
target $\Gamma_\R(s)$, $\zeta$ and even $\chi$ & $1$ & $-0.0416667$ & $0.0145833$ & ---\\
$\Gamma_\R(s+1)$, odd $\chi$ & $1$ & $-0.0416667$ & $0.0145833$ & match (parity)\\
inverted oscillator, half line, even & $-1$ & $-0.0416667$ & $0.0145833$ & \textbf{match}\\
inverted oscillator, half line, odd & $-1$ & $-0.0416667$ & $0.0145833$ & \textbf{match}\\
$c=1$ matrix model / $2$d string \cite{MoorePlesserRamgoolam} & $-1$ & $-0.0416667$ & $0.0145833$ & \textbf{match}\\
Gauss duplication of the target & $1$ & $-0.0416667$ & $0.0145833$ & match (duplication)\\
inverted oscillator, \emph{full} line & $-2$ & $-0.166667$ & $0.233333$ & fails $I_2$, $\times4$\\
modular surface $\Gamma\backslash\mathbb H$ \cite{Iwaniec} & $1$ & $-0.166667$ & $0.233333$ & fails $I_2$, $\times4$\\
JT / Liouville-QM wall & $4$ & $+0.333333$ & $-0.266667$ & fails $I_2$, sign\\
$H_3^+$ ($m$-basis), any level & $-\frac43$ & $+0.333333$ & $-0.266667$ & fails $I_2$, sign\\
bulk Liouville, $b=1$ & $8$ & $+1.33333$ & $-4.26667$ & fails $I_2$, $\times32$\\
FZZT boundary, $b=1$ & $4$ & $+0.333333$ & $-0.266667$ & fails $I_2$, sign\\
cigar $SL(2,\R)/U(1)$, $k=\frac{14}3$ & $-1.5$ & $-0.0416667$ & $-0.0557292$ & $I_2$ exact, fails $I_4$\\
cigar, $k=\frac{782}3$, $n=7$ & $-0.0155$ & $-0.0416667$ & $-0.267731$ & $I_2$ exact, fails $I_4$\\
\bottomrule
\end{tabular}
\end{center}
...
nothing that matters. The cigar is the interesting failure: its shifts
$\frac12+\frac{|n|\mp kw}2$ are tunable, so $I_2$ can be hit exactly --- at small
level and at large --- and neither solution survives $I_4$.
...
model row is its Gauss-duplication image, not an independent candidate, as it
must be, the free-fermion and half-line problems being the same problem.
...
Eisenstein scattering matrix \cite{Iwaniec} is $\varphi(s)=\xi(2s-1)/\xi(2s)$, whose poles are at
$s=\rho/2$: the resonances \emph{are} the zeros. It fails by a factor of four. On
$\re s=\frac12$ the functional equation and reality give
$\xi(2s-1)=\overline{\xi(2s)}$, so $\varphi$ is unimodular with
$\arg\varphi(\frac12+ir)=-2\arg\xi(1+2ir)$, whose $\Gamma$ content is
$\Gamma(\frac12+ir)$ --- shift $\frac12$, not $\frac14$. Hence in $\tau=2r$,
\[
 \mathcal T(\tau)=\log\frac\tau{2\pi}-\frac1{6\tau^2}+O(\tau^{-4}),\qquad
 I_2=-\tfrac16 .
\]
There is no paradox: the resonances lie off the line, at $\re s=\frac14$, while
the delay is read on it, where $\varphi$ is governed by $\xi$ at the edge
$u=1+2ir$ of the strip --- a region in which $\zeta$ has no zeros at all and
$\arg\zeta(1+2ir)$ is bounded. The zeros contribute nothing to the delay, and the
archimedean factor is $\Gamma_\R(2s)$ rather than $\Gamma_\R(s)$, which is exactly
the doubling that costs the factor of four.

Two consequences, and they cut in opposite directions. A pass is weak evidence:
the half-line inverted oscillator matches every invariant exactly and its spectrum
has nothing to do with the zeros. A failure is not evidence of absence: the
modular surface has the zeros as resonances and fails. The delay test is a
measurement of the archimedean factor on the critical line, and of nothing else.
\end{remark}
...
\emph{zeros} either. Both are invisible to every asymptotic order.
\end{remark}
...
The $(n_j,a_j,\beta_j)$ triples of \S\ref{ssec:survey} are readings of the quoted
amplitudes; the arithmetic on them is a computation, and the cigar rows are the
ones most in need of independent re-derivation. The one-parameter family and its
