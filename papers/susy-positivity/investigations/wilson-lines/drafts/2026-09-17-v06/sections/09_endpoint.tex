...
\begin{remark}[Flatness concerns the slice, not the space of contours]
\label{rem:fibre}
Proposition~\ref{prop:flat} is about a \emph{two-parameter} family. The space of
contours is infinite dimensional and the conformal group acts on it with finitely
many parameters, so flatness of a two-dimensional slice says nothing about the
rest. Put usefully: if a correspondence between contour deformations and motions
in $(\omega,L)$ exists it is many-to-one, and the fibre --- contour deformations
that do not move $(\omega,L)$ --- is untouched here. That fibre is exactly where
the shape term of the non-Abelian Stokes variation acts, a deformation at fixed
endpoints being an area deformation. So Theorem~\ref{thm:abelian} constrains
motion along the slice and not the shape term, which acts on the fibre. Whether
it acts trivially there is uncomputed: trivially, and the correspondence is
consistent; nontrivially, and it is overdetermined, which is a condition on a
candidate rather than a freedom.
\end{remark}
