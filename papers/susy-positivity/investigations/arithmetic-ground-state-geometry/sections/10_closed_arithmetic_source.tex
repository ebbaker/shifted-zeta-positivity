...
\subsection{The exact boundary correction is bounded but noncompact}
Translate the interval to $(0,L)$ and use its Neumann cosine modes.
For $a>0$ set
\[
u_a(x)=e^{-ax},\qquad v_a(x)=e^{-a(L-x)},\qquad r_a=e^{-aL},
\]
\begin{equation}\label{boundary:mass}
\mathcal K_a=
\frac{u_au_a^*+v_av_a^*+r_a(u_av_a^*+v_au_a^*)}{1-r_a^2},
\qquad \mathcal K^{(J)}=\sum_{k<J}\mathcal K_{a_k}.
\end{equation}
The finite sums are positive and have rank at most $2J$.

\begin{theorem}\label{boundary:operator}
For every $L>0$, these sums converge strongly to a bounded positive
operator $\mathcal K_L$, and
\begin{equation}\label{boundary:domains}
\begin{split}
T_L&=B(H_{\rm N})+\mathcal K_L,\\
\Dom T_L&=\Dom B(H_{\rm N}),\qquad
\mathcal D_{\log,L}=\Dom B(H_{\rm N})^{1/2}.
\end{split}
\end{equation}
Finite cosine sums are an operator core. The essential norm satisfies
\begin{equation}\label{boundary:essential}
\norm{\mathcal K_L}_{\rm ess}=\pi/2,
\qquad \norm{\mathcal K_L-\mathcal K^{(J)}}\ge\pi/2
\quad\text{for every finite }J.
\end{equation}
\end{theorem}
\begin{proof}
The free compressed resolvent has kernel $e^{-a|x-y|}/(2a)$,
whereas the Neumann Green function is
\[
R_{{\rm N},a}(x,y)=
...
Since the gamma summand is $(2/a)I-2aR$, this gives the finite-tower
identity with the positive sign in \eqref{boundary:mass}.
Its two-by-two coefficient matrix has eigenvalues $(1\mp r_a)^{-1}$,
so each correction is positive.

Put $n(t)=e^{-t/2}/(1-e^{-2t})$. Summing the kernels gives
\[
\mathcal K_L(x,y)=n(x+y)+n(2L-x-y)+\mathcal R_L(x,y),
\]
where $\mathcal R_L$ is bounded: all extra image paths have length
at least $L$, and denominators are bounded below by $1-e^{-L}$.
For example their absolute sum is at most
$2[n(2L)+n(L)]/(1-e^{-L})$.
As $n(t)-(2t)^{-1}$ is bounded on $(0,2L]$, it follows that
\begin{equation}\label{boundary:carleman}
\mathcal K_L=\tfrac12(C_{\rm left}+C_{\rm right})+
\text{an operator with a bounded kernel},
\end{equation}
where the two Carleman kernels are $(x+y)^{-1}$ and
$(2L-x-y)^{-1}$. The weighted Schur test uses
\[
\int_0^\infty\frac{y^{-1/2}}{x+y}\,dy=\pi x^{-1/2}
\]
to bound either operator by $\pi$. The same positive kernel bound
uniformly bounds the partial towers. Increasing bounded positive
operators converge strongly, and the finite-tower identity gives
\eqref{boundary:domains} by monotone form limits. Bounded perturbation
then preserves the operator domain and the finite cosine core.

Split the interval at $L/2$. In \eqref{boundary:carleman} all cross
blocks and the opposite endpoint's kernel on each diagonal block
are bounded kernels and hence compact. Modulo compact operators,
the result is a direct sum of two half-Carleman operators on
$(0,L/2)$, proving essential norm at most $\pi/2$.
For the reverse inequality test the half-line Carleman operator on
$\phi_R(t)=t^{-1/2}1_{[1,R]}(t)/\sqrt{\log R}$.
Logarithmic coordinates give convolution kernel
$[2\cosh((u-v)/2)]^{-1}$, whose integral is $\pi$; the Rayleigh
quotient tends to $\pi$ as $R\to\infty$.
For fixed $R$, dilate this unit vector into
$(\epsilon,\epsilon R)$. It converges weakly to zero, its Carleman
quotient is unchanged, and every compact quadratic contribution
vanishes. First let $\epsilon\downarrow0$, then $R\to\infty$.
This proves the reverse essential-norm inequality. Subtracting the
finite-rank $\mathcal K^{(J)}$ cannot change the essential norm.
\end{proof}
The Carleman model is classical \cite{YafaevCarleman}; the argument
above includes the norm facts needed here. Noncompactness of this
bounded correction is consistent with compact resolvent of the
unbounded $T_L$. It also persists after any finite-codimension
compression. Thus finite mass truncation cannot approximate this
boundary operator in operator norm.

\subsection{Explicit low-input columns with a vanishing tail}
The preceding obstruction has a useful narrower alternative.
Write $\omega_j=j\pi/L$, $e_j(x)=\nu_j\cos(\omega_jx)$, with
$\nu_0=L^{-1/2}$ and $\nu_j=\sqrt{2/L}$ for $j>0$.
\begin{proposition}\label{boundary:columns}
For each mass,
\begin{equation}\label{boundary:column}
\mathcal K_a e_j=
\frac{\nu_j a}{a^2+\omega_j^2}(u_a+(-1)^jv_a).
\end{equation}
Opposite parities have zero matrix element. For equal parities,
\begin{equation}\label{boundary:entries}
\ip{e_i}{\mathcal K_Le_j}
=2\nu_i\nu_j\sum_{k\ge0}
\frac{a_k^2(1-(-1)^j e^{-a_kL})}
{(a_k^2+\omega_i^2)(a_k^2+\omega_j^2)}.
\end{equation}
For the first $N$ modes,
\begin{equation}\label{boundary:column-tail}
\norm{(\mathcal K_L-\mathcal K^{(J)})P_N}
\le\sqrt{\frac{2(2N-1)}L}
       (a_J^{-1/2}+a_J^{-3/2})\longrightarrow0.
\end{equation}
\end{proposition}
\begin{proof}
Integration gives
$\ip{u_a}{e_j}=\nu_ja(1-(-1)^jr_a)/(a^2+\omega_j^2)$ and
$\ip{v_a}{e_j}=(-1)^j\ip{u_a}{e_j}$.
Substitution into \eqref{boundary:mass} cancels its denominator
and gives \eqref{boundary:column}; integration again gives
\eqref{boundary:entries}.
Each column term has norm at most $\sqrt2\nu_ja^{-3/2}$, since
$\norm{u_a},\norm{v_a}\le(2a)^{-1/2}$.
The decreasing-series integral estimate gives
$\sum_{k\ge J}a_k^{-3/2}\le a_J^{-3/2}+a_J^{-1/2}$.
Sum squared column bounds, using
$\sum_{j<N}\nu_j^2=(2N-1)/L$.
\end{proof}
This controls a finite-input response, not the full operator tail.
The distinction will be used to estimate the actual mixed block.
