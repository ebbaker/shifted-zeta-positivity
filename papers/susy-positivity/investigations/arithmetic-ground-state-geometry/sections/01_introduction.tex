...
The next analytical step resolves the gamma boundary coupling
exactly. Its difference from the diagonal Neumann model is bounded
but noncompact, with essential norm $\pi/2$. Finite mass sums fail
in operator norm, while their action on the low-input space has an
explicit vanishing tail. This yields a mixed-response estimate with
the entire residual retained. A separate exact prime-discrepancy
identity shows that replacing the prime measure by its smooth
leading density, with the gamma term, contact and poles unchanged,
produces a negative form at length two. The actual signed arithmetic
information is therefore essential to any remaining comparison.
