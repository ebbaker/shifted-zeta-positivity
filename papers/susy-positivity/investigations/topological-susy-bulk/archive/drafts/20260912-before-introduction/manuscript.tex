...
Let $\mathcal E[U]=\norm{\mathcal C U}_{\mathcal H}^2$ be an independently specified positive energy and let a linear trace map $\operatorname{Tr}$ extract the prescribed input. If for each $f$ there is a linear minimizing extension $U_f$ with $\operatorname{Tr}U_f=f$, then
\begin{equation}
...
\end{equation}
The whole block, not merely $H_{ii}$, supplies positivity. Subtracting a desired kernel after minimization is not an independent positive construction.
