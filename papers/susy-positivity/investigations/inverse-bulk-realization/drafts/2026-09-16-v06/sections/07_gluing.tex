...
the domination is saturated. The check program registered against this
subsection works in a basis of $40$ indicator cells on $I_L$ and gives
$\min\operatorname{spec}(Q_L;K_+)=5.5\times10^{-4}$, $4.1\times10^{-4}$,
$3.7\times10^{-4}$ and $3.0\times10^{-4}$ at $L=1$, $3/2$, $2$ and $3$.

A step basis is poorly adapted to this quantity, because the transforms of its
elements decay only like $\tau^{-1}$ and so cannot avoid the lowest zero
ordinate. Every Rayleigh quotient is an upper bound for the true value, and a
smooth basis does far better. Taking $\phi_k(x)=(1-(2x/L)^2)^k$ and assembling
$Q_L$ from its spectral form \eqref{eq:spectral-target}, so that the
cancellation recorded in Section~\ref{subsec:spectral} never occurs, twelve
such modes give
\begin{equation}\label{eq:saturation-values}
 \min\operatorname{spec}(Q_L;K_+)\;\leq\;6.4\times10^{-10},\;\;
 2.5\times10^{-14},\;\; 1.7\times10^{-17},\;\; 1.2\times10^{-24}
\end{equation}
at the same four values of $L$, and the bound continues to fall as the basis is
enlarged. The values in \eqref{eq:saturation-values} are recorded in the
investigation's notes and are not among the registered check programs; they use
arbitrary-precision arithmetic and a truncated sum over zero ordinates, and
truncating that sum only lowers them. In the cell basis, with $400$ cells, the
number of generalized eigenvalues below $10^{-3}$ is $2$, $6$, $10$ and $14$ at
$L=1$, $3/2$, $2$ and $3$.

A candidate mechanism must therefore produce a \emph{critical} contraction
rather than a strict one, whose spectrum piles up at one on a subspace growing
with $L$; a mechanism yielding a uniform strict contraction, or more generally
any hypothesis supplying a margin uniform in $L$, would be producing a false
statement. All of these are one-sided subspace values, as after
Corollary~\ref{cor:domination}: above one they would certify that the
domination fails, below one they certify nothing.

\begin{remark}[The lowest eigenvalue at small $L$]\label{rem:suzuki}
Suzuki studies the same lowest eigenvalue directly, and unconditionally
\cite{Suzuki}. Writing the localized form as $\ip{A_af}f$ for a self-adjoint
$A_a$ on $L^2(-a,a)$ with $a=L/2$, he proves that $A_a$ is the Friedrichs
extension of an explicit symmetric operator, that its lowest eigenvalue
$\lambda_a$ is continuous in $a$, and that for sufficiently small $a$ it is
positive, simple, and
\begin{equation}\label{eq:suzuki-small}
 \lambda_a=\log(1/a)+\mu_1-\log(2\pi)+\psi(2)-1+O(a).
\end{equation}
None of this assumes RH. Its leading behaviour is the archimedean symbol of the
present paper evaluated at the interval's own frequency scale: adding
\eqref{eq:contact} to \eqref{eq:gamma-growth} gives
\begin{equation}\label{eq:symbol-log}
 b(\tau^2)+w_0=\log\frac{|\tau|}{2\pi}+O(|\tau|^{-2}),
\end{equation}
and $\tau\sim1/a$ reproduces the first two terms of \eqref{eq:suzuki-small},
the constant included. The positivity in \eqref{eq:suzuki-small} is a
small-interval statement and does not reach the range in which the domination
above is saturated.
\end{remark}

\subsection{The target as a jump form, and a necessary condition}
\label{subsec:jump}
The archimedean and prime parts of \eqref{eq:target} are the continuous and
atomic parts of one nonnegative measure. Adding and subtracting the value of the
prime symbol at $\tau=0$ puts everything but the pole term into
L\'evy--Khinchine form.

\begin{proposition}[Jump form of the target]\label{prop:jump}
For $f\in C_c^\infty(I_L)$ and $F=E_Lf$ set
\begin{align}
 \mu_L&=2\gammak(r)\dd r
  +2\!\!\sum_{m\log p<L}\!\!(\log p)p^{-m/2}\delta_{m\log p},
 &\gamma_L&=w_0-2\!\!\sum_{m\log p<L}\!\!(\log p)p^{-m/2},
 \label{eq:levy-measure}
\end{align}
and
\begin{equation}\label{eq:jump-energy}
 \mathcal E_L[F]=\int_0^\infty\big[\norm F^2-\re\ip F{U_rF}\big]\dd\mu_L(r).
\end{equation}
Then $\mu_L\geq0$, $\mathcal E_L\geq0$, and
\begin{equation}\label{eq:jump-decomposition}
 Q_L[f]=\mathcal E_L[E_Lf]+\gamma_L\norm f^2+P_L[f].
\end{equation}
\end{proposition}
\begin{proof}
...
Write $S_L[f]=\mathcal E_L[E_Lf]+\gamma_L\norm f^2$, so that $Q_L=S_L+P_L$.
Away from the diagonal the kernel of $S_L$ is $-\mu_L$, which is nonpositive:
$S_L$ is of Stieltjes type, and the single measure $\mu_L$ carries the
archimedean density and the prime atoms together. This is the presentation in
which they are one object rather than two channels, which
Theorem~\ref{thm:two-channel} requires of any source.

\begin{proposition}[A necessary condition]\label{prop:necessary}
Suppose $S_L$ has a normalized ground state $\psi$ with $S_L[\psi]=-\alpha_L$,
$\alpha_L>0$. If $Q_L\geq0$ on $C_c^\infty(I_L)$ then
\begin{equation}\label{eq:necessary}
 \alpha_L\;\leq\;2\big|\ip{\cosh(x/2)}f\big|^2\Big|_{f=\psi}\;\leq\;
 2\norm{\cosh(x/2)}^2_{L^2(I_L)}=2\sinh(L/2)+L.
\end{equation}
\end{proposition}
\begin{proof}
By \eqref{eq:parity-split},
$0\leq Q_L[\psi]=-\alpha_L+2|\ip{\cosh(x/2)}\psi|^2-2|\ip{\sinh(x/2)}\psi|^2$.
Drop the last term and apply Cauchy--Schwarz, using
$\int_{I_L}\cosh^2(x/2)\dd x=L/2+\sinh(L/2)$.
\end{proof}

\begin{remark}[What the condition is worth, and what it is not]
\label{rem:necessary}
Two things make \eqref{eq:necessary} usable. First it is sharp. A scan of
$7713$ values of $L$ in $[1/2,10]$ on $4000$ cells, with the prime thresholds
sampled on both sides, gives least relative margin $9.5\times10^{-4}$ at
$L=5.3396$ and least absolute margin $0.0187$; there is no crossing. Second,
$\alpha_L$ is bounded \emph{from below} without diagonalizing anything. The
Galerkin value of $\alpha_L$ in any finite basis is such a bound, since a
Rayleigh minimum over a subspace is at least the minimum over the space; and in
the cell basis $S_L=d_LI-N_L$ with $N_L$ entrywise nonnegative and $d_L$
constant, so $\alpha_L$ is a Perron-root deficit and Collatz--Wielandt gives,
for every strictly positive $u$,
\begin{equation}\label{eq:cw}
 \alpha_L\;\geq\;\frac1h\Big(\min_i\frac{(N_Lu)_i}{u_i}-d_L\Big).
\end{equation}
A positive $u$ making the right side of \eqref{eq:cw} exceed $2\sinh(L/2)+L$
would show that $Q_L$ is not positive, hence that RH fails. The constant
function already gives $\alpha_L\geq3.85$ at $L=2$, against a threshold of
$4.35$.

The condition is necessary and not sufficient, and the gap is quantitative
rather than technical. It tests $Q_L$ on the one direction in which $S_L$ is
negative and the pole form is positive; that direction is close to
$\cosh(x/2)$, with cosine $0.995$ at $L=1$ and $0.9998$ at $L=4$. On its
orthogonal complement $S_L$ is bounded below only by its second eigenvalue,
which is $2.5\times10^{-4}$ at $L=2$ and $2.2\times10^{-4}$ at $L=3$, while the
pole form still contributes $-2|\ip{\sinh(x/2)}f|^2$, of size up to
$2\sinh(L/2)-L$, which is $0.35$ and $1.26$ at those two values. The whole
difficulty of the criterion lives on that complement, and a bound on an extreme
eigenvalue does not reach it.
\end{remark}
