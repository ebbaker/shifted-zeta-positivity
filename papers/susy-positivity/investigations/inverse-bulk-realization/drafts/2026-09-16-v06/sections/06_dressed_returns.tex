...
\subsection{Which output states the insertion can reach}
\label{subsec:reachable}
The preparation of Lemma~\ref{lem:one-magnetic} produces one particular state
on the output sector. It is worth recording how much of that state is forced by
the representation and how much is a choice of dressing, because the answer
governs what a weight identity in this sector can be taken to show.

On the zero-charge sector an electric preparation $\eta=D(v)\Omega_q$ has
wavefunction $\eta(w)=C_qD(w)\chi_q(w)$, and \eqref{eq:schur-actions} gives
\begin{equation}\label{eq:output-rule}
 (u_-\eta)_{-1}(\zeta)=(1+\zeta)\,\eta(q^{-1}\zeta).
\end{equation}

\begin{proposition}[Reachable output states]\label{prop:reachable}
Let $\psi$ be analytic on $\{|\zeta|<\rho\}$ for some $\rho>1$, and set
$\eta(w)=\psi(qw)/(1+qw)$. Then $\eta=D(v)\Omega_q$ for
\begin{equation}\label{eq:reach-dressing}
 D(w)=\frac{\psi(qw)\,(-q^3w;q^2)_\infty}{C_q},
\end{equation}
which is analytic on $\{|w|<\rho/q\}$ with $\rho/q>q^{-1}$; the graph-domain
argument of Lemma~\ref{lem:one-magnetic} applies to $D$ unchanged, and the
resulting output state on the sector $m=-1$ is $\psi$. Conversely every output
state arises in this way. Hence the states reachable by one negative magnetic
insertion, from an electric preparation whose dressing is analytic on a disk of
radius exceeding $q^{-1}$, are exactly the functions analytic on some disk of
radius exceeding one.
\end{proposition}
\begin{proof}
Since $\chi_q(w)^{-1}=(-qw;q^2)_\infty=\prod_{j\geq0}(1+q^{2j+1}w)$ has
$(1+qw)$ as its $j=0$ factor,
\[
 C_qD(w)\chi_q(w)
 =\psi(qw)\frac{\prod_{j\geq1}(1+q^{2j+1}w)}{\prod_{j\geq0}(1+q^{2j+1}w)}
 =\frac{\psi(qw)}{1+qw}=\eta(w).
\]
The tail product is entire, so the singularities of $D$ are those of
$\psi(qw)$, at modulus at least $\rho/q$. Choose $R$ strictly between $q^{-1}$
and $\rho/q$. The Taylor polynomials $D_N$ converge uniformly on $|w|\leq R$,
hence $D_N(q^{-1}\zeta)\to D(q^{-1}\zeta)$ uniformly on $|\zeta|\leq qR$, a
disk containing the closed unit disk, and $\chi_q(q\zeta)$ is bounded there
because its nearest pole has modulus $q^{-2}$; the proof of
Lemma~\ref{lem:one-magnetic} then applies verbatim, with $D_N(v)\Omega_q\to\eta$
in norm and $u_-D_N(v)\Omega_q\to\delta_{m,-1}\psi$ in norm. Finally
\eqref{eq:output-rule} gives
$(1+\zeta)\eta(q^{-1}\zeta)=(1+\zeta)\psi(\zeta)/(1+\zeta)=\psi(\zeta)$, and
the same equation determines $\eta$ from $\psi$.
\end{proof}

The factor $(1+\zeta)$ inserted by $u_-$ is therefore not a constraint on the
output. It is cancelled, for every target $\psi$, by the $j=0$ factor of the
vacuum's Pochhammer, which the dressing \eqref{eq:dressing} removes and another
dressing may retain. The $(1+\zeta)$ surviving in \eqref{eq:magnetic-state} is
the $j=0$ factor of that dressing's own numerator $(-qw;q^2)_\infty$ evaluated
at $w=q^{-1}\zeta$, not a feature of the operator. No pole is crossed in either
case: the cancellation happens between the dressing's analyticity and the
identity $(1+\zeta)\chi_q(q^{-1}\zeta)=\chi_q(q\zeta)$ already used in
\eqref{eq:graph-cancellation}.

Two consequences. The mirror reference of
Section~\ref{subsec:subtraction} is reachable, which is
Corollary~\ref{cor:mirror-schur} below. And producing a prescribed weight in
this sector is not by itself evidence of arithmetic structure in the
representation, since any weight whose square root is analytic past the unit
circle can be produced by choosing the dressing. What
Theorem~\ref{thm:schur-prime} establishes beyond the weight is that one fixed
$q$ serves every prime and that the insertion is justified on a checked domain.
Candidate preparations should be assessed on those points, and on the
compression of Section~\ref{subsec:gammacompression}, rather than on the
appearance of a prime weight in a positive sector.
