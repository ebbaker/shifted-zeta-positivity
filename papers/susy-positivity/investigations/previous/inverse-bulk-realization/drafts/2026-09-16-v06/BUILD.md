...
complete buildable manuscript, PDF, build record, validation tool and the six
check programs with their preserved records, and writes `SNAPSHOT.json`.
...
`--replay` additionally runs the six current check programs and compares the
resulting JSON with their preserved records. The last two are floating-point
computations, so an exact replay match is expected on one platform and is not a
promise of bit-identical output across Python or libm versions. It does not certify proofs,
