...
qualified construction problems. Ten small standard-library Python
...
\texttt{check\_mirror\_dressing.py} & 638 & Reachable output states, the mirror dressing, and its weight \\
\texttt{check\_gamma\_compression.py} & 32,037 & The identity $Q_L=K_+-T_L$, the sign change of $c_L$, and one-sided saturation \\
\bottomrule
\end{tabular}
\end{center}
The 112,816 finite cases are supporting checks, not 112,816 independent
...
\texttt{check\_mirror\_dressing.py} works with truncated power series in exact
rational arithmetic. It verifies \eqref{eq:output-rule} for both the dressing
of Lemma~\ref{lem:one-magnetic} and the mirror dressing
\eqref{eq:mirror-dressing}, the ratio of the two electric states, the
telescoping of the vacuum Pochhammer at four truncations, the Laurent
coefficients of the two weights in the ratios $2/(1+r)$ and $2/(1-r)$ with
geometric ratio $r$, the orthogonality of the two output states --- the overlap
truncated at $N$ is exactly $r^{2N}$ --- and the reachability of three targets
unrelated to the prime references. Its one floating-point item, labelled as
such, is the geometric rate $r$ at which the truncated dressings' images
converge at the radius $q^{-1}$ where the magnetic action evaluates them.

\texttt{check\_gamma\_compression.py} verifies
Proposition~\ref{prop:gamma-compression} as a matrix identity on indicator
cells at ten values of $L$ between $1/2$ and $5$, agreeing to
$7.5\times10^{-16}$, and rebuilds the mirror matrices independently, checking
them against \eqref{eq:subtraction}. It locates the sign change of $c_L$ at
$p=13$ exactly, confirms $T_L\geq0$ at every sampled $L$, and reports the
saturation of the domination. That last column is one sided in the direction
opposite to a Rayleigh minimum: a subspace value is a \emph{lower} bound for
$\norm\Pi$, so a value meaningfully above one would certify that the domination
fails and a value below one certifies nothing. The values quoted in
Section~\ref{subsec:gammacompression} come from a Fourier--Galerkin space,
which resolves the near-degeneracy better than the cell basis recorded here.
