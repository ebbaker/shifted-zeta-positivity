...
|---|---|---|
| `PAPER_first_slab_positivity_preprint_v1.tex` | `papers/first-slab-positivity/first_slab_positivity.tex` (+ PDF) | released, v1.0 |
| `PAPER_psi_omega_margin.tex`, `PAPER_psi_omega_concise.tex` | `papers/psi-omega-margin/` (+ PDFs) | **working drafts**, committed 9 Sept 2026; author block filled |
| `c1_xi_values.py` … `c8_prime_side_chi.py` (recovered) | `papers/psi-omega-margin/code/` | 27 Aug recomputation set |
| `PROGRESS_psi_omega_next_steps.md` | `papers/psi-omega-margin/supplementary/NEXT_STEPS.md` | redacted copy (checklist items 1, 4, 5 applied; sign-off pending) |
| `PAPER_omega_string.tex` | `papers/omega-string/` (+ PDF, `fig_density.png` regenerated) | **working draft v5**, committed 8 Sept 2026; author block filled |
| round-15 verification scripts (reconstructed) | `papers/omega-string/{fig_density,v1_identities,v2_thmE_kasahara}.py` | |
| `PAPER_defect_depth.tex` | `papers/defect-depth/` (+ PDF) | **working draft v1**, committed 9 Sept 2026 |
| `lab_ihara_core.py`, `lab_ihara_experiments.py` | `papers/defect-depth/code/` | laboratory record through round 5 |
| `PAPER_rh_detector.tex` | `papers/rh-detector/` (+ PDF) | **working draft**, committed 9 Sept 2026; separate thread, predates the Suzuki program |
| `h1`, `h3`, `h4`, `d1`, `d4`, `v1` + `zeros1001.json`, `beta_zeros.json` (regenerated 25 Aug) | `papers/rh-detector/code/` | smoke-tested 9 Sept |
...
| `NOTE_psi_omega_margin.md`, `ROUND13_psi_omega_recheck.md`, `NOTE_oneside_criterion.md`, `ROUND14_concise_draft_check.md` | `notes/` | the substance is summarised in `papers/psi-omega-margin/supplementary/DERIVATION_AND_VERIFICATION.md` |
...
- Paper 3, round-6 inline runs: amplitude certification, `s₀` five-point scan,
  unscaled real-ζ suite, envelope bracketing (`papers/defect-depth/STATUS.md`, item 5);
